...
export class AnalyticsService {
  ...
  async collect(
    ...
    const rows = events.slice(0, MAX_BATCH)
      ...
      .map((e) => this.eventRepository.create({
        name: e.name,
    ...
    this.eventRepository.save(rows)
      .catch(err => this.logger.warn(`collect insert failed: ${err?.message}`));
    return rows.length;
  }
  ...
  private safeTimestamp(raw?: string): Date {
    ...
    if (parsed > now + 60_000 || parsed < now - 7 * 24 * 60 * 60 * 1000) return new Date(now);
  ...
  async summary(days: number): Promise<Record<string, unknown>> {
    ...
    const [
      ...
      q(`SELECT to_char(date_trunc('day', date_created), 'YYYY-MM-DD') AS day,
                ...
                COUNT(*)::int AS total
         FROM "post" WHERE date_created >= $1 GROUP BY 1 ORDER BY 1`, [since]),

      q(`SELECT to_char(date_trunc('day', date_created), 'YYYY-MM-DD') AS day,
                COUNT(*)::int AS created,
                COUNT(*) FILTER (WHERE status = 'ACCEPTED')::int AS accepted
         FROM "booking" WHERE date_created >= $1 GROUP BY 1 ORDER BY 1`, [since]),

      q(`SELECT to_char(date_trunc('day', occurred_at), 'YYYY-MM-DD') AS day,
                COUNT(DISTINCT COALESCE("userId"::text, anon_id))::int AS value
         FROM "analytics_event" WHERE occurred_at >= $1 GROUP BY 1 ORDER BY 1`, [since]),
      ...
      q(`SELECT COALESCE(props->>'message', 'unknown') AS message,
  ...
  async prune(): Promise<void> {
    try {
      ...
      const { affected } = await this.eventRepository.delete({ occurred_at: LessThan(cutoff) });
      if (affected) this.logger.log(`Pruned ${affected} analytics events older than ${RETENTION_DAYS}d.`);
    } catch (err) {
