...
export class BookingService {
  ...
  private rethrowBookingConflict(err: any): never {
    ...
    if (constraint === 'EX_booking_accepted_no_overlap') {
      throw new BadRequestException({
        ...
        message: 'Dates overlap an already accepted booking',
      });
    }
    throw err;
  }
  ...
  async create(customerId: string, dto: CreateBookingDto) {
    ...
    if (!schema?.has_rental_status)
      throw new BadRequestException({
        code: 'BOOKING_CATEGORY_UNSUPPORTED',
        message: 'This category does not support bookings',
      });
    ...
    const saved = await this.bookingRepository
      .save(booking)
      .catch((err) => this.rethrowBookingConflict(err));

    this.events?.emitBookingEvent(
      post.user.id,
      SOCKET_EVENTS.BOOKING_REQUESTED,
      { bookingId: saved.id, postId: post.id },
    );
    this.notifications
      .notifyUsers(
        [post.user.id],
  ...
  async respond(
    ...
    if (booking.provider?.id !== providerId)
      throw new ForbiddenException({
        ...
        message: 'Not your booking to respond to',
      });
    ...
    if (accept) {
      ...
      if (new Date(booking.end_date) < today) {
        throw new BadRequestException({
          code: 'BOOKING_DATE_PAST',
          message: 'These dates have already passed',
        });
      }
    ...
    this.events?.emitBookingEvent(
      booking.customer.id,
      SOCKET_EVENTS.BOOKING_RESPONDED,
      {
        bookingId: saved.id,
        status: saved.status,
      },
  ...
  async busyRanges(
    ...
    const rows = await this.bookingRepository
      ...
      .where('b.postId = :postId AND b.status = :accepted', {
        postId,
        accepted: BookingStatus.ACCEPTED,
      })
  ...
  async expireStaleBookings(): Promise<void> {
    try {
      ...
      if (n)
        this.logger.log(
          `expireStaleBookings: expired ${n} unanswered request(s)`,
        );
    } catch (err) {
  ...
  async promptReviews(): Promise<void> {
    try {
      const due = await this.bookingRepository
        .createQueryBuilder('b')
        .leftJoin('b.post', 'post')
        .addSelect(['post.id'])
        .leftJoin('b.customer', 'customer')
        .addSelect(['customer.id'])
        .leftJoin('b.provider', 'provider')
        .addSelect(['provider.id'])
