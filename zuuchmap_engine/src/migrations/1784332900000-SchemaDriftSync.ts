...
export class SchemaDriftSync1784332900000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "post" ADD COLUMN IF NOT EXISTS "expires_at" TIMESTAMP`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_post_subcategory" ON "post" ("subcategory")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_user_email" ON "user" ("email")`,
    );
    await queryRunner.query(
      `ALTER TABLE "post" DROP CONSTRAINT IF EXISTS "FK_5c1cf55c308037b5aca1038a131"`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "post" DROP COLUMN IF EXISTS "expires_at"`,
    );
  }
}
