...
const NEW_CATEGORIES = [
  {
    ...
    subcategories: [
      ...
      },
      {
        value: 'spare_parts',
        display: 'Сэлбэг',
        labels: { mn: 'Сэлбэг', en: 'Spare Parts', zh: '配件', ru: 'Запчасти' },
      },
    ],
    fields: [
      {
        ...
        labels: {
          mn: 'Үйлдвэрлэгч/Брэнд',
          en: 'Manufacturer/Brand',
          zh: '制造商/品牌',
          ru: 'Производитель/Бренд',
        },
        type: 'text',
        filterable: true,
      },
      {
        key: 'model',
        label: 'Загвар',
        labels: { mn: 'Загвар', en: 'Model', zh: '型号', ru: 'Модель' },
        type: 'text',
        filterable: true,
      },
      {
      ...
      },
      {
        key: 'condition',
        label: 'Байдал',
        labels: { mn: 'Байдал', en: 'Condition', zh: '成色', ru: 'Состояние' },
        type: 'select',
        filterable: true,
  ...
  },
  {
    ...
    subcategories: [
      ...
      },
      {
        ...
        labels: {
          mn: 'Хүнд даацын тээвэр',
          en: 'Heavy Haul',
          zh: '大件运输',
          ru: 'Негабаритные перевозки',
        },
      },
      {
    ...
    fields: [
      ...
      {
        key: 'service_route',
        label: 'Үйлчлэх чиглэл',
        labels: {
          mn: 'Үйлчлэх чиглэл',
          en: 'Service route/area',
          zh: '服务路线',
          ru: 'Маршрут',
        },
        type: 'text',
  ...
  },
  {
    ...
    labels: {
      mn: 'Зураг төсөл, инженеринг',
      en: 'Design & Engineering',
      zh: '设计与工程',
      ru: 'Проектирование и инжиниринг',
    },
    ...
    subcategories: [
      ...
      },
      {
        ...
        labels: {
          mn: 'Бүтээцийн инженер',
          en: 'Structural Engineering',
          zh: '结构工程',
          ru: 'Конструкторские работы',
        },
      },
      {
        value: 'surveying',
      ...
      },
      {
        ...
        labels: {
          mn: 'Хөрсний шинжилгээ',
          en: 'Soil Testing',
          zh: '土壤检测',
          ru: 'Исследование грунта',
        },
      },
      {
        value: 'permits',
  ...
  },
  {
    ...
    labels: {
      mn: 'Уул уурхайн үйлчилгээ',
      en: 'Mining Support',
      zh: '矿业服务',
      ru: 'Горнодобывающие услуги',
    },
    ...
    subcategories: [
      {
        ...
        labels: {
          mn: 'Өрөмдлөг, тэсэлгээ',
          en: 'Drilling & Blasting',
          zh: '钻爆作业',
          ru: 'Буровзрывные работы',
        },
      },
      {
        value: 'earthworks',
      ...
      },
      {
        ...
        labels: {
          mn: 'Кемпийн үйлчилгээ',
          en: 'Camp Services',
          zh: '营地服务',
          ru: 'Обслуживание лагерей',
        },
      },
      {
        value: 'maintenance',
    ...
    fields: [
      ...
      {
        key: 'certifications',
        label: 'Гэрчилгээ, стандарт',
        labels: {
          mn: 'Гэрчилгээ, стандарт',
          en: 'Certifications',
          zh: '认证标准',
          ru: 'Сертификаты',
        },
  ...
  },
  {
    ...
    subcategories: [
      ...
      },
      {
        ...
        labels: {
          mn: 'Өвөлжилтийн бэлтгэл',
          en: 'Winterization',
          zh: '冬季防护',
          ru: 'Подготовка к зиме',
        },
      },
    ],
    fields: [
...
const SUB_RENAMES: Array<
  [string, string, string, string, Partial<Record<string, string>>?]
> = [
...
export class CategoryExpansion1784333600000 implements MigrationInterface {
  ...
  private async loadRow(
    ...
    const rows = await queryRunner.query(
      `SELECT "id", "subcategories", "fields" FROM "category_schema" WHERE "key" = $1`,
      [key],
    );
    return rows[0];
  }
    ...
    for (const cat of NEW_CATEGORIES) {
      await queryRunner.query(
        ...
        [
          cat.key,
          cat.label,
          JSON.stringify(cat.labels),
          cat.icon,
          cat.color,
          cat.sort_order,
          cat.has_rental_status,
          cat.has_availability_dates,
          cat.has_price,
          cat.default_price_unit,
  ...
  private async applyRenamesAndAdditions(
    queryRunner: QueryRunner,
    dir: 'up' | 'down',
  ): Promise<void> {
    ...
    for (const key of keys) {
      ...
      for (const [fieldKey, wrong, fixed] of FIELD_RENAMES) {
        ...
        // `Field` is Record<string, unknown>, so the localized label bag has to
        // be named before it can be read or written.
        const labels = field?.labels as Record<string, string> | undefined;
        if (!field || labels?.mn !== from) continue;
        labels.mn = to;
      ...
      if (dir === 'up') {
        ...
        row.subcategories = [
          ...(row.subcategories ?? []),
          ...addedSubs.filter((s) => !existing.has(s.value)),
        ];
      } else {
        const remove = new Set(addedSubs.map((s) => s.value));
        row.subcategories = (row.subcategories ?? []).filter(
          (s) => !remove.has(s.value),
        );
      }
      ...
      if (dir === 'up') {
        ...
        row.fields = [
          ...(row.fields ?? []),
          ...addedFields.filter((f) => !existing.has(f.key)),
        ];
      } else {
