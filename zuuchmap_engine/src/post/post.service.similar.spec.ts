...
describe('PostService.findSimilar', () => {
  const anchor = { id: 7, category: 'vehiclerent', district: 'BZD', province: 'UB', price_amount: '150000', user: null };
  ...
  const makeService = (items: unknown[] = []) => {
    ...
    const postRepo = {
      ...
      manager: { query: jest.fn(async (..._args: any[]): Promise<any[]> => []) },
    };
    const categoryService = { getCategories: jest.fn(async () => [{ key: 'vehiclerent', has_rental_status: true }]) };
    const svc = new PostService(
      postRepo as any, {} as any, {} as any, categoryService as any, {} as any, undefined as any,
    );
  ...
  it('ranks same district, then same province, then price distance, then newest', async () => {
    ...
    expect(qb.addSelect).toHaveBeenCalledWith(expect.stringContaining('post.district = :district'), 'loc_rank');
    expect(qb.addSelect).toHaveBeenCalledWith(expect.stringContaining('ABS(post.price_amount'), 'price_dist');
    ...
    expect(qb.setParameters).toHaveBeenCalledWith({ district: 'BZD', province: 'UB', price: 150000 });
  });

  it('stays in the category, excludes the anchor and only returns live posts', async () => {
    const { svc, qb } = makeService();
    await svc.findSimilar(7);
    expect(qb.where).toHaveBeenCalledWith('post.category = :category', { category: 'vehiclerent' });
    expect(qb.andWhere).toHaveBeenCalledWith('post.id != :id', { id: 7 });
    expect(qb.andWhere).toHaveBeenCalledWith('post.approval_status = :approved', { approved: 'APPROVED' });
    expect(qb.andWhere).toHaveBeenCalledWith('post.status = :active', { active: 'ACTIVE' });
  });
  ...
  it('attaches busy_dates to rental results', async () => {
    const { svc, postRepo } = makeService([{ id: 8, category: 'vehiclerent', user: null }]);
    const today = new Date();
    postRepo.manager.query.mockResolvedValueOnce([{ postId: 8, start_date: today, end_date: today }]);
...
describe('expandBusyDates', () => {
  ...
  it('expands a range into days and clips to the 14-day window', () => {
    const out = expandBusyDates(
      [{ postId: 1, start_date: '2026-08-20T00:00:00Z', end_date: '2026-09-30T00:00:00Z' }],
      today,
    );
  ...
  it('drops bookings entirely outside the window', () => {
    const out = expandBusyDates([{ postId: 1, start_date: '2026-10-01', end_date: '2026-10-05' }], today);
...
describe('PostService.attachBusyDates', () => {
  const makeService = (schemas: unknown[], rows: unknown[] = []) => {
    ...
    const svc = new PostService(
      { manager: { query } } as any, {} as any, {} as any,
      { getCategories: jest.fn(async () => schemas) } as any, {} as any, undefined as any,
    );
    return { svc, query };
  };
  ...
  it('only queries for posts in has_rental_status categories, and leaves others untouched', async () => {
    ...
    const posts: any[] = [{ id: 1, category: 'vehiclerent' }, { id: 2, category: 'jobvacancy' }];
    await svc.attachBusyDates(posts);
    expect(query).toHaveBeenCalledTimes(1);
    expect((query.mock.calls[0] as any[])[1][0]).toEqual([1]);
  ...
  it('skips the query entirely when nothing on the page is bookable', async () => {
    const { svc, query } = makeService([{ key: 'jobvacancy', has_rental_status: false }]);
    await svc.attachBusyDates([{ id: 2, category: 'jobvacancy' }] as any);
...
describe('PostService.update — previous_snapshot for re-approval', () => {
  const makePost = (over: Record<string, unknown> = {}) => ({
    id: 1, category: 'vehiclerent', user: { id: 'owner' },
    title: 'Old title', details: 'd', price_amount: '100', price_unit: 'day',
    attributes: { a: 1 }, images: ['x.jpg'], subcategory: 'truck', province: 'UB', district: 'BZD',
    approval_status: 'APPROVED', previous_snapshot: null, rejection_field: 'title',
    ...over,
  });
  const makeService = (post: any) => {
    const postRepo = { findOne: jest.fn(async () => post), save: jest.fn(async (x: any) => x) };
    const svc = new PostService(
      postRepo as any, {} as any, {} as any,
      { getCategories: jest.fn(async () => []) } as any, {} as any, undefined as any,
    );
    return { svc, postRepo };
  };
  ...
  it('snapshots the approved version and clears the rejection field on a content edit', async () => {
    ...
    const updated = await svc.update(1, { title: 'New title' } as any, [], 'owner');
    expect(updated.approval_status).toBe('PENDING');
    expect(updated.previous_snapshot).toEqual({
      title: 'Old title', details: 'd', price: 100, price_unit: 'day', attributes: { a: 1 },
      images: ['x.jpg'], subcategory: 'truck', province: 'UB', district: 'BZD',
    });
  ...
  it('keeps the first snapshot across a second edit before re-approval', async () => {
    ...
    const post = makePost({ approval_status: 'PENDING', previous_snapshot: first, title: 'Draft 1' });
    const { svc } = makeService(post);
    const updated = await svc.update(1, { title: 'Draft 2' } as any, [], 'owner');
  ...
  it('does not snapshot a post that was never approved', async () => {
    ...
    const { svc } = makeService(post);
    const updated = await svc.update(1, { title: 'Fixed' } as any, [], 'owner');
  ...
  it('leaves an operational-only edit approved and unsnapshotted', async () => {
    const post = makePost();
    const { svc } = makeService(post);
    const updated = await svc.update(1, { status: 'RENTED' } as any, [], 'owner');
    expect(updated.approval_status).toBe('APPROVED');
    expect(updated.previous_snapshot).toBeNull();
  });

  it('snapshotOf coerces the decimal price to a number', () => {
    expect(snapshotOf(makePost({ price_amount: '99.50' }) as any).price).toBe(99.5);
    expect(snapshotOf(makePost({ price_amount: null }) as any).price).toBeNull();
  });
});
