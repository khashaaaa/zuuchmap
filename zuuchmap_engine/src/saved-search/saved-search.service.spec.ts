...
describe('matchesSavedSearch', () => {
  ...
  it('q prefix-matches each term, case-insensitively', () => {
    ...
    expect(
      matchesSavedSearch({ ...post, title: null, details: null }, { q: 'x' }),
    ).toBe(false);
  });
  ...
  it('q searches details, not only the title', () => {
    ...
    expect(
      matchesSavedSearch({ ...post, details: null }, { q: 'жолооч' }),
    ).toBe(false);
  });
  ...
  it('attrs: equality, with or without the attr. prefix, compared as strings', () => {
    expect(matchesSavedSearch(post, { attrs: { 'attr.brand': 'Howo' } })).toBe(
      true,
    );
    expect(matchesSavedSearch(post, { attrs: { brand: 'Howo' } })).toBe(true);
    expect(matchesSavedSearch(post, { attrs: { 'attr.year': 2019 } })).toBe(
      true,
    );
    expect(
      matchesSavedSearch(post, { attrs: { 'attr.brand': 'Shacman' } }),
    ).toBe(false);
    expect(matchesSavedSearch(post, { attrs: { 'attr.missing': 'x' } })).toBe(
      false,
    );
    expect(matchesSavedSearch(post, { attrs: { 'attr.brand': '' } })).toBe(
      true,
    );
  });
  ...
  it('attrs: _min/_max are numeric bounds', () => {
    expect(
      matchesSavedSearch(post, { attrs: { 'attr.capacity_min': '20' } }),
    ).toBe(true);
    expect(
      matchesSavedSearch(post, { attrs: { 'attr.capacity_min': 30 } }),
    ).toBe(false);
    expect(
      matchesSavedSearch(post, { attrs: { 'attr.capacity_max': 25 } }),
    ).toBe(true);
    expect(
      matchesSavedSearch(post, { attrs: { 'attr.capacity_max': 24 } }),
    ).toBe(false);
    expect(matchesSavedSearch(post, { attrs: { 'attr.brand_min': 1 } })).toBe(
      false,
    );
    expect(
      matchesSavedSearch(
        { ...post, attributes: null },
        { attrs: { 'attr.capacity_min': 1 } },
      ),
    ).toBe(false);
  });
});
